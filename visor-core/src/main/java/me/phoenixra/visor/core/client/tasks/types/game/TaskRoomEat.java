...
@RegisterVisorTask
public class TaskRoomEat extends VisorTaskBase {
    private static final String ID = "consume";

    @Getter
    private static TaskRoomEat instance;
    ...
    public TaskRoomEat(@NotNull VisorAddon owner) {
        super(owner);
        instance = this;
    }
    ...
    protected void onRun(LocalPlayer player) {
        VRClientPose roomPose = ClientContext.player
    ...
    private Vec3 calculateHandPosition(VRClientPose roomPose,
                                       ControllerHand hand) {
        Vec3 basePos = ClientContext.rawPlayerPose.getControllerData(hand)
                .getPositionHistory()
