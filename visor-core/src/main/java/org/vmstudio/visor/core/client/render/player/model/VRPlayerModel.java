...
public class VRPlayerModel<T extends LivingEntity> extends PlayerModel<T> {
    ...
    private final Vector3f tempAnimV = new Vector3f();
    private final Vector3f tempAnimV2 = new Vector3f();
    ...
    public static void animateVRModel(
        ...
        var poseRelative =  vrPlayer.getPoseData(PlayerPoseType.RELATIVE);
        var offhandPose =  poseRelative.getOffhand();
        var mainHandPose =  poseRelative.getMainHand();
        var worldScale =  vrPlayer.getPoseData(PlayerPoseType.TICK).getWorldScale();
        float bodyYaw = poseRelative.getBodyYaw();
        ...
        var headQuat = poseRelative.getHmd().getRotation().getNormalizedRotation(new Quaternionf());
        // head pivot
        if (!swimming) {
            headQuat.transform(0F, -0.2F, 0.1F, tempV2);
            tempV2.mul(vrPlayer.getFullHeightScale() * worldScale);
        } else {
            // no pivot offset when swimming
            tempV2.zero();
        }
        tempV2.add(poseRelative.getHmd().getPosition());
        ...
        model.attackArm = attackArm;
        model.bodyYaw = bodyYaw;
        model.laying = laying;
        model.layAmount = layAmount;
        model.bodyScale = bodyScale;
        model.armScale = armScale;
        model.legScale = legScale;
        model.xRot = xRot;
    }
