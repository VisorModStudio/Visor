# Relicensing: LGPL-3.0 to MPL-2.0

In August 2026 Visor is changing its license from the GNU Lesser General Public
License 3.0 to the Mozilla Public License 2.0. The change takes effect with the
first release published under MPL-2.0.

- The change is **not retroactive**: all releases published before it remain
  available under LGPL-3.0.
- Contributions merged after the change are covered by the terms in
  [CONTRIBUTING.md](CONTRIBUTING.md), so this file only concerns contributions
  made while the project was LGPL-3.0.

## How to give consent

Open a pull request **from your own GitHub account** that adds one line to the
Consents section below (following the format of the existing entries) and
removes your name from the Awaiting section. The commit - authored by the same
account that made your contributions - is the record of your consent.

## Consents

- I, Phoenix-Ra, author of contributions made to this repository under that
  account, agree to relicense all my past contributions under the Mozilla
  Public License 2.0. - 2026-08-10

## Awaiting

- [ ] Weever1337
- [ ] Ninix44

## Consent not required

- misterbubb - his only contribution
  ([`6aa21d0`](https://github.com/Phoenix-Ra/Visor/commit/6aa21d00747f02b88a69116ccea998949a561079),
  Vivecraft conflict detection) was replaced by an independent
  reimplementation in
  [`a76b71f`](https://github.com/Phoenix-Ra/Visor/commit/a76b71faae0805a4f9b8d114f0cccacf4f14701f)
  before the license change, so no code from it is part of the relicensed
  tree. The original contribution remains available in the git history under
  LGPL-3.0.
