...
//@TODO Vivecraft Legacy
public class TextScrollWidget extends AbstractWidget {

    private final int SCROLL_BAR_WIDTH = 5;
    private final int PADDING = 5;
    private final List<FormattedCharSequence> FORMATTED_CHARS;
    ...
    public TextScrollWidget(int x, int y, int width, int height, String text) {
        ...
        this.FORMATTED_CHARS = Minecraft.getInstance().font.split(Component.literal(text),
            width - this.SCROLL_BAR_WIDTH * 2);
    ...
    public TextScrollWidget(int x, int y, int width, int height, Component text) {
        ...
        this.FORMATTED_CHARS = Minecraft.getInstance().font.split(text, width - this.SCROLL_BAR_WIDTH * 2);
        initScroll();
    }
    ...
    private void initScroll() {
        ...
        this.currentLine = 0;
        this.scrollSteps = this.FORMATTED_CHARS.size() - this.maxLines;
        this.scrollSteps = Math.max(this.scrollSteps, 0);
        this.scrollBarSize = this.scrollSteps == 0 ? this.height - 2 :
            (int) (Math.max(this.FORMATTED_CHARS.size(), this.maxLines) / (float) (this.scrollSteps) * 12);
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // draw text
        for (int line = 0; line + this.currentLine < this.FORMATTED_CHARS.size() && line < this.maxLines; line++) {
            guiGraphics.drawString(
                    Minecraft.getInstance().font,
                    this.FORMATTED_CHARS.get(line + this.currentLine),
                    getX() + this.PADDING,
                    getY() + this.PADDING + line * 12,
                    0xFFFFFF
            );
        }
        ...
        if (isFocused() || this.isHovered) {
            ...
            guiGraphics.fill(
                getX() + this.width - this.SCROLL_BAR_WIDTH - 2,
        ...
        // draw scroll bar
        guiGraphics.fill(
            getX() + this.width - this.SCROLL_BAR_WIDTH - (isFocused() || this.isHovered ? 1 : 2),
    ...
    public void onClick(double mouseX, double mouseY) {
        if (mouseX >= getX() + this.width - this.SCROLL_BAR_WIDTH && mouseX <= getX() + this.width &&
            mouseY >= getY() && mouseY <= getY() + this.height)
        {
            this.scrollDragActive = true;
            if (this.maxLines < this.FORMATTED_CHARS.size()) {
    ...
    public Style getMouseoverStyle(double mouseX, double mouseY) {
        ...
        if (lineIndex >= 0 && lineIndex < this.FORMATTED_CHARS.size()) {
            FormattedCharSequence line = this.FORMATTED_CHARS.get(lineIndex);
